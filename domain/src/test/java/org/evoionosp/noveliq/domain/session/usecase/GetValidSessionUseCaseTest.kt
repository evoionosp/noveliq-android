...
class GetValidSessionUseCaseTest {
     ...
     */
    @Test
    fun `refreshes up front when the stored token has already expired`() = runTest {
        val store = FakeSessionStore(session(expiresInSeconds = -3600))
        val authRepository = FakeAuthRepository(
            LoginResult.Success(LoginData("new-access", "refresh-2", "user-1"))
        )
    ...
    @Test
    fun `refreshes when the token is inside the expiry skew window`() = runTest {
    ...
    @Test
    fun `returns null when the expired session cannot be recovered`() = runTest {
        val store = FakeSessionStore(session(expiresInSeconds = -3600))
    ...
    @Test
    fun `hands back the expired session when the refresh cannot reach the server`() = runTest {
        val store = FakeSessionStore(session(expiresInSeconds = -3600))
    ...
    @Test
    fun `treats an unknown expiry as usable`() = runTest {
    ...
    @Test
    fun `returns null when there is no session at all`() = runTest {
